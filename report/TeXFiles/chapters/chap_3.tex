...
\noindent
1. \textbf{Input:} The user interacts with the GUI (e.g., clicking "Inject Vehicle").
\newline
2. \textbf{Command:} The GUI sends a command to the \texttt{VehicleManager} or \texttt{TrafficLightWrapper}.
\newline
3. \textbf{Simulation:} The \texttt{VehicleSimulator} executes the command via TraCI during the next simulation step.
\newline
4. \textbf{Update:} SUMO computes the new state.
\newline
5. \textbf{Fetch:} The \texttt{VehicleManager} fetches the updated vehicle positions and statuses.
\newline
6. \textbf{Render:} The \texttt{MapView} receives the immutable \texttt{VehicleState} snapshot and redraws the scene.
\medskip

\noindent
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.95\textwidth]{../assets/architecture_diagram.png}
